  ...
  try {
    ...
    const predictPriority = (description) => {
      return new Promise((resolve, reject) => {
        execFile(
          "python",
          ["D:/Academics/SEM 5/SDE/Taskmanager/server/models/ml/predict.py", description],
          (error, stdout, stderr) => {
            if (error) {
              ...
              resolve(stdout.trim()); // Priority result from Python script
            }
          }
        );
      });
    ...
    // Determine priority, either given or predicted
    let taskPriority = priority && priority.toUpperCase() !== "NONE"
    ? priority.toLowerCase()
    : await predictPriority(description);
    ...
    text += ` The task priority is set at ${taskPriority} priority, so check and act accordingly. The task date is ${new Date(
      date
...
//     const task = await Task.create({
//       title,// Include description here
//       team,
//       stage: stage.toLowerCase(),
//       date,
